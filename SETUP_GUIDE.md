...
- **Tagged friends show up as one long garbled number instead of separate people** (e.g. `111866889883005337000000000...`) — Google Sheets auto-detected the `tagged_friends`/`userId` cell as a Number and merged the comma-separated list into one value (the comma got read as a thousands separator). Current `Code.gs` guards against this on every write, so this shouldn't happen with an up-to-date deployment — if you see it: (1) confirm you've pasted the *latest* `Code.gs` and pushed a new deployment version (editing the script alone doesn't update the live URL — see the callout after step 4), then (2) for any entry that was already corrupted before that, re-select its "Tag friends" and save again to overwrite the cell with a correctly-formatted value — the corrupted precision can't be recovered automatically, only overwritten.
