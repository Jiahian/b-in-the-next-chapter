...
- Opening the app's link inside Telegram's own browser blocks Google sign-in outright, so the app detects that case and offers a way to continue in a real browser before the sign-in screen ever appears (see `DECISIONS.md` for how).
